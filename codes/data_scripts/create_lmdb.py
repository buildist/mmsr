...
def vimeo90k(mode):
    ...
    BATCH = 5000  # After BATCH images, lmdb commits, if read_all_imgs = False
    if mode == 'GT':
        img_folder = 'D:/datasets/vimeo_septuplet/train'
        lmdb_save_path = 'D:/datasets/vimeo_septuplet/train.lmdb'
        txt_file = 'D:/datasets/vimeo_septuplet/sep_trainlist.txt'
        H_dst, W_dst = 256, 448
    elif mode == 'LR':
    ...
    pickle.dump(meta_info, open('D:/datasets/vimeo_septuplet/train.pkl', "wb"))
    print('Finish creating lmdb meta info.')
...
def REDS(mode):
    #### read all the image paths to a list
    print('Reading image path list ...')
    all_img_list = data_util._get_paths_from_images('D:/Datasets/REDS/train_HR/b')
    keys = []
    for img_path in all_img_list:
    ...
    # meta_info['resolution'] = '{}_{}_{}'.format(channel, H_dst, W_dst)
    meta_info['keys'] = keys
    pickle.dump(meta_info, open('D:/Datasets/REDS/train_HR.pkl', "wb"))
    print('Finish creating lmdb meta info.')
